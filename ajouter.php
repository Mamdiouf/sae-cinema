...
$livres       = $pdo->query("SELECT id, titre, auteur FROM livres ORDER BY titre")->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $id_livre       = ($_POST['id_livre'] ?? '') !== '' ? (int)$_POST['id_livre'] : null;
    ...
    if ($titre === '' || $id_realisateur === 0) {
        ...
        try {
            ...
            // le film
            $req = $pdo->prepare("
                INSERT INTO films (titre, annee, duree, affiche, note, id_realisateur, id_livre)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ");
            $req->execute([$titre, $annee ?: null, $duree ?: null, $affiche ?: null, $note, $id_realisateur, $id_livre]);
...
<form class="box" method="post" action="ajouter.php">
    ...
    <label for="id_livre">Livre d'origine (si adaptation)</label>
    <select id="id_livre" name="id_livre">
        <option value="">— Aucun (film original) —</option>
        <?php foreach ($livres as $li): ?>
            <option value="<?= (int)$li['id'] ?>">
                <?= htmlspecialchars($li['titre'].' — '.$li['auteur']) ?>
            </option>
        <?php endforeach; ?>
    </select>
